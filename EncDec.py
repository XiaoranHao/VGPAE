...
class Encoder(nn.Module):
    def __init__(self, in_channels, latent_dim1,
                 latent_dim2, activation, img_size=64, n_channels=None):
...
class BUCovEncoder(Encoder):
    def __init__(self, layer=conv_block, mlp_h_dim=300, *args, **kwargs):
        super(BUCovEncoder, self).__init__(*args, **kwargs)
...
class CovDecoder(Encoder):
    def __init__(self, layer=convt_block, *args, **kwargs):
        super(CovDecoder, self).__init__(*args, **kwargs)
        self.last_layer_size = math.ceil(self.img_size / (2 ** len(self.n_channels))) ** 2
        self.last_layer_width = math.ceil(self.img_size / (2 ** len(self.n_channels)))
        modules = []
        in_channels = self.in_channels
...
class CovEncoder(Encoder):
    def __init__(self, layer=conv_block, *args, **kwargs):
        super(CovEncoder, self).__init__(*args, **kwargs)
        self.last_layer_size = math.ceil(self.img_size / (2 ** len(self.n_channels))) ** 2
        self.last_layer_width = math.ceil(self.img_size / (2 ** len(self.n_channels)))
        modules = []
        in_channels = self.in_channels
